...
module.exports = msg => {
  ...
  if (msg.guild) {
    ...
    if (txt === "bb help me") {
      msg
        .reply(
          `Hey! I currently work only in DM, so type **bb text me** in a text-channel and you will receive a private message in your inbox!\nOnce you've opened our private chat you can use only these commands:\n${supportedCommandsToString()}`
        )
        .catch(() => {
          msg.author.send(
            `**I haven't enough permission** to reply you in ${
              msg.guild.name
            } - #${
              msg.channel.name
    ...
    if (user.lastCommand.cmd) {
      ...
      if (txt === "cancel") {
        if (user.isPlaying) {
          let wasMatched = db.setLastCommand(user.id, {
            cmd: "matchmakingCancelled",
            arg: ""
          });
          if (wasMatched)
            chat.send(`I'll ask your opponent if he wants to cancel too!`);
          else chat.send(`Ohh ok, let me know when you want to play!`);
          return;
        }
      ...
      if (txt === "end") {
        if (user.isPlaying) {
          if (db.setLastCommand(user.id, { cmd: "askForReport", arg: "" }))
            chat.send(
              `Okk, **who has won?** Type 'I'/'we' if you've won, 'he'/'they' if you've lost or 'draw' if you've drew.`
            );
          else
            chat.send(
              `You can't report if you aren't matched! If you want to quit type 'cancel'.`
            );
        } else {
      ...
      switch (user.lastCommand.cmd) {
        case "askForGame":
          ...
          if (game1 != "") {
            chat.send(
              `Ohh nice! For ${game1.name} there are ${
                game1.modes.length
              } modes: ${modesToString(
                game1.modes
        ...
        case "askForMode":
          ...
          if (game2.modes.includes(txt)) {
            chat.send(
              `Perfect! Do you want me to start looking for a match on ${
                game2.name
              } - ${txt}? Yes or no?`
            );
            db.setLastCommand(user.id, {
              cmd: "askForMatchmaking",
              arg: { name: game2.name, mode: txt }
            });
          } else {
            chat.send(
              `Wait, ${txt} it's not a valid mode for ${
                game2.name
        ...
        case "askForMatchmaking":
          ...
          if (txt === "y" || txt === "yes" || txt === "yep") {
            chat.send(
              `Nice, I'm looking for a match! I'll let you know when I'll find a game 🔍`
            );
            ...
            chat.send(
              `Okk, I've just cancelled the operation! Do you want to edit game/mode? Yes or no?`
            );
            db.setLastCommand(user.id, {
              cmd: "cancelMatchmaking",
              arg: game3
            });
          } else chat.send(`You can just say 'yes' or 'no'.`);
          break;
        case "cancelMatchmaking":
          let game4 = user.lastCommand.arg;
          if (txt === "y" || txt === "yes" || txt === "yep") {
            chat.send(
              `No problem, what game do you want to play?\nThese are the supported games: ${supportedGamesToString()}`
            );
            db.setLastCommand(user.id, { cmd: "askForGame", arg: "" });
          } else if (txt === "n" || txt === "no" || txt === "nope") {
            chat.send(`Ohh ok, let me know when you want to play!`);
        ...
        case "askForReport":
          if (txt === "i" || txt === "we") {
            ...
            chat.send(
              `You can just say 'I'/'we' (if you've won), 'he'/'they' (if you've lost) or 'draw' (if you've drew).`
            );
          break;
        case "askForCancel":
          if (txt === "y" || txt === "yes" || txt === "yep") {
            ...
            chat.send(
              `I understand! Talk with your opponent, he's to give you the win or he must play!`
            );
        ...
        case "started":
          if (txt === "play") {
            if (!user.isPlaying) {
              ...
              chat.send(
                `**You are already in a match!** To quit type 'cancel'.`
              );
            }
          } else {
            chat.send(
              `Uhmmm don't know what you want :c Please type 'help' in order to get the list of the available commands!`
            );
          }
          break;
      }
      ...
      if (txt === "play") {
        ...
        if (txt === "whomadethis") {
          return chat.send(
            "AndyIsCool5463#5230 edited this bot for personal use. Originally forked from https://github.com/acerbisgianluca/BaffBot/"
          );
        }
        chat.send(
          `Hey ${
...
function supportedGamesToString() {
  ...
  supportedGames.forEach((el, i) => {
    str += el.name;
    if (i != supportedGames.length - 1) str += ", ";
    else str += "**";
  });
